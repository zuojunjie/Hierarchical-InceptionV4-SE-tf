...
def _get_streaming_metrics(prediction, label, num_classes):
  ...
  with tf.name_scope("eval"):
    ...
    confusion = _create_local('confusion_matrix',shape=[21499,100],dtype=tf.int64)

    # Create the update op for doing a "+=" accumulation on the batch

    confusion_update = confusion.assign(confusion)
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    [image, label] = provider.get(['image', 'label_1'])
    ...
    if FLAGS.moving_average_decay:
      ...
      variables_to_restore[tf_global_step.op.name] = labels
    else:
      variables_to_restore = slim.get_variables_to_restore()

    predictions = tf.argmax(logits, 1)
    AAAA.append(predictions)
    labels = tf.squeeze(labels)
    reporter = EvalReporter(images, predictions, labels,logits)
...
#	'labels':labels,
    ...
    for i in range(1):
      aaaa,bbbb=slim.evaluation.evaluate_once(
        master=FLAGS.master,
        checkpoint_path=checkpoint_path,
        logdir=FLAGS.eval_dir,
        num_evals=num_batches,
        eval_op=eval_ops,
        final_op=[labels,predictions],
